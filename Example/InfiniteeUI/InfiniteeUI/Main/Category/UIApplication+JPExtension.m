...
+ (void)load {
    if (@available(iOS 13.0, *)) {
        ...
        dispatch_once(&onceToken, ^{
            [self jp_swizzleInstanceMethodsWithOriginalSelector:@selector(setStatusBarStyle:) swizzledSelector:@selector(jp_setStatusBarStyle:)];
            [self jp_swizzleInstanceMethodsWithOriginalSelector:@selector(setStatusBarStyle:animated:) swizzledSelector:@selector(jp_setStatusBarStyle:animated:)];
        });
    }
}
